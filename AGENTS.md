...
This is a project to control an animatronic Halloween display that is a system of clusters of animatronic eyes in tin cans, wherein each cluster controls up to eight animatronic “eyeballs in cans.”  We will call each pair of eyes in a can a “Derrick”.  Each cluster can power and control up to eight Derricks.   The entire system can consist of up to 15 nodes, with a single master control unit (MCU) running QLC+ and communicating with the clusters via ESP-NOW   
...
Each Derrick can be activated and controlled independently.  The Derrick will also have a section of an addressable RGB LED strip connected in series throughout the cluster such that each of the eight Derricks can light up independently to illuminate the eyes in a variety of colors to match the mood of the animation sequence.  Each cluster of Dereks (called a "cluster") will have two DFPlayer MP3 units and audio amplifiers and speakers, and a PWM servo controller capable of driving multiple servos. 
...
Each cluster’s audio can be operated in dependently, or in sync with the other clusters.  Each MP3 module will be loaded with an SD card containing numbered audio files.  The module will play sounds as each Derrick is actuated, or as an animation sequence involving multiple derricks is performed.

The following are also configurable - the Number of Derrick clusters, number of Derricks in each specific cluster, and calibration of the eye location of each Derrick in each cluster - such that each pair of eyes can be calibrated to locate any position in 360 degrees around the system, with each maxing out at their most left or right position.  Each eye can effectively look at a portion of the 360 degrees around the system.
